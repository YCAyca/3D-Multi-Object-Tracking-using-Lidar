...
def load_oxts(oxts_file):
    ...
    directory = os.path.dirname(os.path.abspath(oxts_file))
    filename = os.path.splitext(os.path.basename(oxts_file))[0]
    ...
    for i,oxts in enumerate(oxts_packets):
        print("VF, VL, wZ frame", i, oxts[8], oxts[9], oxts[19])
        vf.append(oxts[8])
        vl.append(oxts[9])
        vz.append(oxts[19])
...
def get_ego_traj(imu_poses, frame, pref, futf, inverse=False, only_fut=False):
 ...
 #   print("imu poses", imu_poses)
    print("num_ frames, frame", num_frames, frame)
    assert frame >= 0 and frame <= num_frames - 1, 'get ego error' 
    ...
    print("start,end,index",start,end,index)
    ...
    if only_fut:
        ...
        return cur_world_xyz, fut_rot_list, left, right
    else:
        return cur_world_xyz, all_rot_list, left, right
...
def egomotion_compensation_ID(traj_id, calib, ego_rot_imu, ego_xyz_imu, left, right, mask=None):
    ...
    traj_id_rect = calib.imu_to_velo(traj_id_imu)
